...
/**
 * ViewHelper to render inlineJS in <footer> section of website
 *
 * @package TYPO3
...
class InlineJsViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	*/
	public function render() {
		...
		$contentObjectUid = $this->arguments['contentObjectUid'];
		...
		switch ( $action ) {
		 ...
		 if (ExtensionManagementUtility::isLoaded('vhs')) {
			...
			if ($settings['moveInlineJsFromHeaderToFooter']) {
				...
				$pageRenderer->addJsInlineCode($name,$function.$inlineJS);
			}
		 }
	}
}
